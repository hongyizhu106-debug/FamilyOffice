...
import { planAReportPdf } from "./tools/planAReportPdf.js";
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      ,
      {
        name: planAReportPdf.name,
        description: planAReportPdf.description,
        inputSchema: planAReportPdf.parameters
      }
    ]
  };
});
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  switch (request.params.name) {
    ...
    case "planA_report_pdf": {
      const report_id = String(request.params.arguments?.report_id);
      const market = String(request.params.arguments?.market);
      const symbol = String(request.params.arguments?.symbol);
      const period = String(request.params.arguments?.period);
      const output = request.params.arguments?.output;
      const compare = request.params.arguments?.compare;
      return await planAReportPdf.run({ report_id, market, symbol, period, output, compare } as any);
    }
