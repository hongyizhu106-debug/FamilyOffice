...
const toolList = [
  ...
  { name: planAReportPdf.name, description: planAReportPdf.description, inputSchema: planAReportPdf.parameters }
];
...
// 静态文件：PlanA PDF/HTML/JSON 输出目录
app.use('/planA/files', express.static(pathResolve(process.cwd(), 'PlanA/_out')));
      ...
      const result = await runWithRequestContext({ tushareToken: token }, async () => {
        switch (name) {
          ...
          case 'planA_report_pdf':
            return await planAReportPdf.run({
